...
clusters <- ggplot(long_cluster_tally, aes(x = lifestage, y = OG_cluster, fill= value)) + 
  ...
  scale_fill_gradient2(low = "white", high = "deepskyblue4", 
                       midpoint = 0.0, 
                       breaks = seq(0, 1.1, 0.1), # adjust the breaks for the maximum proportion
                       limits = c(0, 1.1))# adjust the breaks for the maximum proportion


clusters

pdf('diff_20_clusters.pdf', width = 4 , height = 8) ## Change name and dimensions here
...
OGs_heat <- ggplot(long_og_tally, aes(x = lifestage, y = OG, fill= value)) + 
  ...
  scale_fill_gradient2(low = "white", high = "deepskyblue4", 
                       midpoint = 0.0, 
                       breaks = seq(0, 2.6, 0.1), #adjust breaks
                       limits = c(0, 2.6))#adjust breaks

OGs_heat

pdf('diff_20_OGs.pdf', width = 4 , height = 7) #change name
